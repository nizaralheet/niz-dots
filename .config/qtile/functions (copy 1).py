...
class CalPopup:
    def __init__(self):
        self.started=True
        self.hidden=True
    def _create_layout(self, highlighted_cal_output):
    ...
    def toggle(self, x=0, y=0):
        ...
        cal_output = (subprocess.check_output("cal", shell=True, text=True)).splitlines()
        # Highlight the current day in the calendar output
        highlighted_cal = []
        for line in cal_output:
        ...
        if not self.started:
            self._create_layout(highlighted_cal_output)
            self.layout.show(qtile=qtile, x=x, y=y)
            self.started = True
            self.hidden = False
        else:
